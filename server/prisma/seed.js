...
// 42 threads so pagination has several pages (page size 10 → 5 pages).
// createdAt is spread so "newest first" ordering is stable and visible.
...
async function main() {
  ...
  for (let i = 1; i <= TOTAL; i++) {
    ...
    const thread = await prisma.thread.create({
      data: {
        ...
        authorId,
        createdAt: day(TOTAL - i), // #01 oldest … #42 newest
      },
    });
  ...
  console.log(`✅ Seeded ${TOTAL} threads (page size 10 → 5 pages)`);
}
